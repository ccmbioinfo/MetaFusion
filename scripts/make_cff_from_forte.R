...
parser$add_argument("-f",'--fusion_caller',type="character",default = NULL)
...
if(nrow(tool_cff) > 0 ){
  tool_cff$sample_name <- opt$sample
...
write.table(tool_cff,opt$output,sep = "\t",row.names = F,quote =  F,col.names =F)
